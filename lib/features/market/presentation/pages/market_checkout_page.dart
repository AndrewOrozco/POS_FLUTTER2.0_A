...
/// Página de checkout / confirmación de venta de Market (KCO).
/// Misma estructura que CanastillaCheckoutPage con color scheme teal.
class MarketCheckoutPage extends StatefulWidget {
  final List<ItemCarrito> carrito;
  final VoidCallback onVentaExitosa;

  const MarketCheckoutPage({
    super.key,
...
class _MarketCheckoutPageState extends State<MarketCheckoutPage> {
  ...
  Future<void> _cargarMediosPago() async {
    ...
    final medios = await _api.obtenerMediosPagoMarket();
    if (mounted) {
      setState(() {
        _mediosPago = medios;
        _cargando = false;
  ...
  Future<void> _cargarConfigFacturacion() async {
    try {
      final config = await _api.obtenerConfigFacturacionMarket();
      if (mounted) {
        setState(() {
  ...
  Future<void> _mostrarDialogoFE() async {
    ...
    final resultado = await showDialog<ClienteConsulta?>(
      ...
      builder: (_) => const _DialogoFacturacionElectronicaMarket(),
    );
  ...
  Future<void> _confirmarVenta({bool esFacturacionElectronica = false}) async {
    ...
    try {
      ...
      final result = await _api.procesarVentaMarket(body);
  ...
  Future<void> _imprimir() async {
    ...
    final result = await _api.imprimirMarket(
      _movimientoId!,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Confirmar Venta - Market',
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                Icon(Icons.receipt_long, color: _accentColor),
        ...
        children: [
            ...
            child: ListView.builder(
              ...
              itemBuilder: (ctx, idx) {
                ...
                return Padding(
                  ...
                  child: Material(
                    color: sel ? _primaryColor.withValues(alpha: 0.12) : Colors.grey.shade50,
                        ...
                        child: Row(
                          children: [
                            Icon(_iconForMedio(mp.descripcion),
                                color: sel ? _primaryColor : Colors.grey.shade600, size: 20),
                            const SizedBox(width: 8),
                            Expanded(child: Text(mp.descripcion,
                                style: TextStyle(
                                  fontWeight: sel ? FontWeight.bold : FontWeight.w500,
                                  fontSize: 13,
                                  color: sel ? _accentColor : Colors.black87,
                                ))),
                            if (sel)
          ...
          if (esEfectivo) ...[
            ...
            Container(
              ...
              child: Row(
                children: [
                  const Text('\$ ', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: _primaryDark)),
                  Expanded(
                    child: Text(
          ...
          // Botón F. ELECTRONICA / FACTURA POS
          Padding(
...
// ============================================================
// Diálogo compacto de Facturación Electrónica (Market)
// ============================================================

class _DialogoFacturacionElectronicaMarket extends StatefulWidget {
  const _DialogoFacturacionElectronicaMarket();

  @override
  State<_DialogoFacturacionElectronicaMarket> createState() =>
      _DialogoFacturacionElectronicaMarketState();
}

class _DialogoFacturacionElectronicaMarketState
    extends State<_DialogoFacturacionElectronicaMarket> {
  final ApiConsultasService _api = ApiConsultasService();
