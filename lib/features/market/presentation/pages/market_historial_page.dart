...
/// Historial de ventas de Market (KCO).
/// Filtros: rango de fechas. Tabla con datos de venta y botón imprimir.
class MarketHistorialPage extends StatefulWidget {
  const MarketHistorialPage({super.key});

  @override
  State<MarketHistorialPage> createState() => _MarketHistorialPageState();
}

class _MarketHistorialPageState extends State<MarketHistorialPage> {
  ...
  Future<void> _cargarHistorial() async {
    ...
    final data = await _api.obtenerHistorialMarket(
  ...
  Future<void> _imprimir(int movimientoId) async {
    ...
    final result = await _api.imprimirMarket(movimientoId);
    if (mounted) {
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: SingleChildScrollView(
          child: DataTable(
            headingRowColor:
                WidgetStateProperty.all(_primaryLight),
            columnSpacing: 16,
            columns: const [
