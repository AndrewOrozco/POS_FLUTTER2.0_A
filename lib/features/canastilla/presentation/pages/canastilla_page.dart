...
import 'canastilla_checkout_page.dart';
import 'canastilla_historial_page.dart';

/// Página principal del módulo Canastilla (tienda de conveniencia).
/// Layout de 2 paneles: Catálogo (izquierda) + Carrito (derecha).
class CanastillaPage extends StatefulWidget {
  const CanastillaPage({super.key});

  @override
  State<CanastillaPage> createState() => _CanastillaPageState();
}

class _CanastillaPageState extends State<CanastillaPage> {
  ...
  Future<void> _cargarCategorias() async {
    final cats = await _api.obtenerCategoriasCanastilla();
  ...
  Future<void> _cargarProductos({bool reset = true}) async {
    ...
    final data = await _api.obtenerProductosCanastilla(
      page: _page,
      pageSize: _pageSize,
  ...
  void _irACheckout() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CanastillaCheckoutPage(
  ...
  void _irAHistorial() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const CanastillaHistorialPage()),
    );
  }
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(Icons.search, color: Color(0xFFFF8F00)),
